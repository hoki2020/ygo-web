Auxiliary={}
aux=Auxiliary
-- Some srvpro builds do not preload procedure.lua automatically.
-- Lazy-load it when missing helpers are accessed (e.g. aux.AddXyzProcedure).
do
	local loaded_procedure=false
	local mt=getmetatable(aux) or {}
	local old_index=mt.__index

	local function ensure_ritual_compat(name)
		if rawget(aux,name)~=nil then
			return rawget(aux,name)
		end
		if name=="AddRitualProcGreaterCode" then
			rawset(aux,name,function(c,code1,...)
				if aux.AddCodeList then aux.AddCodeList(c,code1) end
				local filter=nil
				if aux.FilterBoolFunction and Card and Card.IsCode then
					filter=aux.FilterBoolFunction(Card.IsCode,code1)
				end
				if aux.AddRitualProcGreater then
					return aux.AddRitualProcGreater(c,filter,...)
				elseif aux.AddRitualProcCode then
					return aux.AddRitualProcCode(c,code1,...)
				elseif aux.AddRitualProc then
					return aux.AddRitualProc(c,filter,...)
				end
			end)
			return rawget(aux,name)
		end
	...
	local function install_proc_proxy(name,fallback)
		local current=rawget(aux,name)
		if type(current)=="function" then
			return
		end
		local proxy
		proxy=function(...)
			if not loaded_procedure and Duel and Duel.LoadScript then
				loaded_procedure=pcall(Duel.LoadScript,"procedure.lua")
			end
			local actual=rawget(aux,name)
			if type(actual)=="function" and actual~=proxy then
				return actual(...)
			end
			if fallback then
				return fallback(...)
		...
		rawset(aux,name,proxy)
	end

	local function ritual_fallback(c,code1,...)
			if aux.AddCodeList then aux.AddCodeList(c,code1) end
			local filter=nil
			if aux.FilterBoolFunction and Card and Card.IsCode then
				filter=aux.FilterBoolFunction(Card.IsCode,code1)
	...
	install_proc_proxy("AddRitualProcGreaterCode",ritual_fallback)
	install_proc_proxy("AddLinkProcedure")
	install_proc_proxy("AddXyzProcedure")
	install_proc_proxy("AddFusionProcFun2")
	install_proc_proxy("AddFusionProcFunRep")
	setmetatable(aux,mt)
end
...
function GetID()
	...
	local code=tonumber((source:match("c(%d+)%.lua")))
	if not code then
		return nil,nil,1
	end
	local self_table=_G["c"..code]
	if self_table==nil then
		self_table={}
		_G["c"..code]=self_table
	end
	local offset=code<100000000 and 1 or 100
	return self_table,code,offset
end
...
function Auxiliary.ExceptThisCard(e)
	local c=e:GetHandler()
	local ok,related=pcall(function() return c:IsRelateToChain() end)
	if not ok then
		ok,related=pcall(function() return c:IsRelateToChain(e) end)
	end
	if ok and related then return c else return nil end
end
