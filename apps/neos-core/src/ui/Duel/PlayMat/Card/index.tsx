...
export const Card: React.FC<{ idx: number }> = React.memo(({ idx }) => {
  ...
  const isMySetCardOnField =
    matStore.isMe(snap.location.controller) &&
    [MZONE, SZONE].includes(snap.location.zone) &&
    facedownPositions.includes(snap.location.position);
        ...
        >
          ...
          <YgoCard
            className={classnames(styles.back, {
              [styles["back-overlay"]]: isMySetCardOnField,
            })}
            isBack
          />
        </div>
