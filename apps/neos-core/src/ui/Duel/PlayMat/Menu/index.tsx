...
export const Menu = () => {
  ...
  useEffect(() => {
    const firstTurnNoBpM2 = turnCount === 1 && matStore.isMe(currentPlayer);
    const endResponse = [
    ...
    setPhaseBind((prev) => {
      ...
      for (const item of newItems) {
        ...
        if (phase === PhaseType.BATTLE) {
          item[3] = !firstTurnNoBpM2;
          item[4] = firstTurnNoBpM2 || !enableBp;
        } else if (phase === PhaseType.MAIN2) {
          item[3] = !firstTurnNoBpM2;
          item[4] = firstTurnNoBpM2 || !enableM2;
  ...
  }, [enableBp, enableM2, enableEp, currentPhase, currentPlayer, turnCount]);
...
const SelectManager: React.FC = () => {
  ...
  return (
    <div className={styles["select-manager"]}>
      <Button
        className={classNames(styles.btn, { [styles.cancel]: cancelable })}
