    ...
    case sqliteCmd.INIT: {
      ...
      if (info) {
        ...
        const releasePromise = pfetch(info.releaseDbUrl, {
          ...
          if (!res.ok) {
            throw new Error(
              `Failed to load release card db: ${res.status} ${res.statusText}`,
            );
          }
        ...
        const preReleasePromise = pfetch(info.preReleaseDbUrl, {
          ...
          .then((res) => {
            if (!res.ok) {
              throw new Error(
                `Failed to load pre-release card db: ${res.status} ${res.statusText}`,
              );
            }
            return res.arrayBuffer();
          })
          .catch((err) => {
            console.warn(
              "pre-release db fetch failed, fallback to release db",
              err,
            );
            return null;
          });
        ...
        return Promise.all([
          ...
          YGODBS.preRelease = new SQL.Database(
            new Uint8Array(preReleaseBuffer ?? releaseBuffer),
          );
